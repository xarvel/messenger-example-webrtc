...
import {MessagesList_messages$key} from './__generated__/MessagesList_messages.graphql';
import {palette} from '../palette';
import {MessagesList_meta$key} from './__generated__/MessagesList_meta.graphql';
...
export const MessagesList: FC<MessagesListProps> = ({
  ...
  const {data, loadPrevious, hasPrevious, loadNext} = usePaginationFragment<
    MessagesListPaginationQuery,
    MessagesList_messages$key
  >(messagesQuery, messagesRef);

  const {viewer, chat} = useFragment(metaInfoQuery, metaRef);

  const NamesMap = chat.participants.reduce((acc, curr) => {
  ...
  return (
    <FlatList
      ...
      ListHeaderComponent={
        isTypingUser ? <IsTypingView name={getName(isTypingUser)} /> : null
      }
      style={{
      ...
      renderItem={({item}) => (
        <MessageItem getSide={getSide} getName={getName} dataRef={item.node} />
      )}
    />
  );
};
